...
export class GitHubQueueService {
    ...
    async addFetchRawTreeDataJob(data: IFetchRawTreeDataJob) {
        ...
        console.log(`[GitHubQueueService] Queued ${FETCH_RAW_TREE_DATA_JOB_NAME} job ${job.id} for repository ${data.repositoryId}`);
        return job;
    }

    async addFetchRawCommitsDataJob(data: IFetchRawCommitsDataJob) {
        const job = await this.fetchProviderDataQueue.add(
            FETCH_RAW_COMMITS_DATA_JOB_NAME,
            data,
            {
        ...
        console.log(`[GitHubQueueService] Queued ${FETCH_RAW_COMMITS_DATA_JOB_NAME} job ${job.id} for repository ${data.repositoryId}`);
        return job;
    }
